...
xmlns:func="http://exslt.org/functions">
	...
	<xsl:template match="/">
		...
		<head>
			...
			<link rel="stylesheet" href="convertcsv.css"/>
		</head>
		<body>
			<h1>
			...
			<!-- VARIABLES -->
			<xsl:variable name="rowPath" select="/root/row">
			</xsl:variable>
			...
			<func:function name="func:displayStock">
				...
				<xsl:variable name ="count" select="position()"/>
					<tr>
						<itemCode><xsl:value-of select="/root/row[$count]/itemCode"/></itemCode>
						<itemDescription><xsl:value-of select="/root/row[$count]/itemDescription"/></itemDescription>

						<xsl:variable name ="currentCount" select="/root/row[$count]/currentCount"/>
